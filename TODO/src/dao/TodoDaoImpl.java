...
public class TodoDaoImpl implements TodoDao {
    ...
    private static final String INSERT_TODOS_SQL = "INSERT INTO todos" +
        "  (title, description, target_date,  is_done, for_id) VALUES " + " (?, ?, ?, ?, ?);";

    private static final String SELECT_TODO_BY_ID = "select id,title,description,target_date,is_done from todos where id =?";
    private static final String SELECT_ALL_TODOS = "select * from todos where for_id=?";
    ...
    public void insertTodo(Todo todo) throws SQLException {
        ...
        try (Connection connection = JDBCUtils.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(INSERT_TODOS_SQL)) {
            ...
            preparedStatement.setInt(5, todo.getfid());
    ...
    public List < Todo > selectAllTodos() {
        ...
        try (Connection connection = JDBCUtils.getConnection();
        	...
        	preparedStatement.setInt(1, LoginDao.getid());
            System.out.println(preparedStatement);
            // Step 3: Execute the query or update query
            ResultSet rs = preparedStatement.executeQuery();
